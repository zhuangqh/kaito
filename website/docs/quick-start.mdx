...
- **GPU nodes available in your cluster** - You have two **mutually exclusive** options:
  ...
  - **Auto-provisioning**: Set up automatic GPU node provisioning for your cloud provider (see [these steps](installation#option-2-auto-provision-gpu-nodes)). **Cannot be used with BYO nodes.**
...
:::info Prerequisites for BYO Nodes
...
2. You have existing GPU nodes in your cluster
...
The nodes will be populated with their specs using [NVIDIA GPU Feature Discovery](https://github.com/NVIDIA/k8s-device-plugin/tree/main/docs/gpu-feature-discovery#deployment-via-helm), and KAITO will verify that your nodes have the proper memory, # GPUS, etc. to run the model.
